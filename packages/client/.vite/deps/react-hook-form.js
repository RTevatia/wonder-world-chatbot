...
//#region ../../node_modules/.bun/react-hook-form@7.72.1+b1ab299f0a400331/node_modules/react-hook-form/dist/index.esm.mjs
...
var useFormControlContext = () =>
   import_react.useContext(HookFormControlContext);
...
function deepEqual(
   object1,
   object2,
   _internal_visited = /* @__PURE__ */ new WeakSet()
) {
...
function useWatch(props) {
   ...
   const getCurrentOutput = import_react.useCallback(
      (values) => {
   ...
   const refreshValue = import_react.useCallback(
      (values) => {
         if (!disabled) {
            const formValues = generateWatchOutput(
               name,
...
function useController(props) {
   ...
   const _registerProps = import_react.useRef(
   ...
   const fieldState = import_react.useMemo(
      () =>
   ...
   const onChange = import_react.useCallback(
      (event) =>
         ...
         if (field && field._f && elm)
            field._f.ref = {
               ...
               setCustomValidity: (message) =>
                  isFunction(elm.setCustomValidity) &&
...
var FormStateSubscribe = ({ control, disabled, exact, name, render }) =>
   render(
      useFormState({
         control,
         name,
...
function isTraversable(value) {
   return (
      Array.isArray(value) || (isObject(value) && !objectHasFunction(value))
   );
}
...
function createFormControl(props = {}) {
   ...
   const handleSubmit = (onValid, onInvalid) => async (e) => {
      ...
      if (_names.disabled.size)
         for (const name of _names.disabled) unset(fieldValues, name);
...
function useFieldArray(props) {
   ...
   const updateValues = import_react.useCallback(
   ...
   const remove = (index) => {
      ...
      !Array.isArray(get(control._fields, name)) &&
         set(control._fields, name, void 0);
   ...
   return {
      swap: import_react.useCallback(swap, [updateValues, name, control]),
      move: import_react.useCallback(move, [updateValues, name, control]),
      prepend: import_react.useCallback(prepend, [updateValues, name, control]),
      append: import_react.useCallback(append, [updateValues, name, control]),
      remove: import_react.useCallback(remove, [updateValues, name, control]),
      insert: import_react.useCallback(insert$1, [updateValues, name, control]),
      update: import_react.useCallback(update, [updateValues, name, control]),
      replace: import_react.useCallback(replace, [updateValues, name, control]),
      fields: import_react.useMemo(
         () =>
...
function useForm(props = {}) {
   ...
   import_react.useEffect(() => {
      props.shouldUnregister &&
         control._subjects.state.next({ values: control._getWatch() });
   }, [control, props.shouldUnregister]);
   import_react.useEffect(() => {
      if (control._proxyFormState.isDirty) {
         const isDirty = control._getDirty();
         if (isDirty !== formState.isDirty)
            control._subjects.state.next({ isDirty });
      }
   ...
   _formControl.current.formState = import_react.useMemo(
